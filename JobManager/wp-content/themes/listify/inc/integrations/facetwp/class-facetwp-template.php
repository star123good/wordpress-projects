...
<div class="facetwp_job_listings">
	...
	public function output_facet_html( $facets = array() ) {
		...
		foreach ( $facets as $key => $facet ) {
			$output[] = '
				<aside class="facetwp-filter facetwp-filter-' . esc_attr( $facet['type'] ) . ' facetwp-filter-' . esc_attr( $facet['name'] ) . ' widget-job_listing-archive">
	...
	public function facetwp_result_count( $output, $params ) {
		if ( 0 === $params['total'] ) {
			return __( 'No results found', 'listify' );
		}
